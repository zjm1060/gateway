 ...
 */
int MQTTPacket_send(networkHandles* net, Header header, char* buffer, size_t buflen, int fre)
{
	int rc;
	size_t buf0len;
	char *buf;
...
#if defined(OPENSSL)
	if (net->ssl)
		rc = SSLSocket_putdatas(net->ssl, net->socket, buf, buf0len, 1, &buffer, &buflen, &fre);
	else
#endif
		rc = Socket_putdatas(net->socket, buf, buf0len, 1, &buffer, &buflen, &fre);
